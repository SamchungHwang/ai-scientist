...
- "Experiment": An outline of the implementation. E.g. which functions need to be added or modified, 
...
If there is nothing to improve, simply repeat the previous JSON EXACTLY after the thought 
nd include "I am done" at the end of the thoughts but before the JSON.
...
def generate_ideas(
    ...
    if skip_generation:
        ...
        try:
            ...
            outftn("No existing ideas found. Generating new ideas.")
        except json.JSONDecodeError:
            outftn("Error decoding existing ideas. Generating new ideas.")
    ...
    for _ in range(max_num_generations):
        if stop_generation:
            outftn("stopping generation")
            return

        outftn(f"Generating idea {_ + 1}/{max_num_generations}")
        try:
            prev_ideas_string = "\n\n".join(idea_str_archive)

            msg_history = []
            outftn(f"Iteration 1/{num_reflections}")
            first_prompt = idea_first_prompt.format(
                task_description=task_description,  # task_description=prompt["task_description"],
                code=code,
            ...
            if num_reflections > 1:
                for j in range(num_reflections - 1):
                    if stop_generation:
                        outftn("stopping generation")
                        return
                    outftn(f"Iteration {j + 2}/{num_reflections}")
                    # print("idea_reflection_prompt=", idea_reflection_prompt)
                    ...
                    if "I am done" in text:
                        outftn(f"Idea generation converged after {j + 2} iterations.")
                        break

            idea_str_archive.append(json.dumps(json_output))
        except Exception as e:
            outftn(f"Failed to generate idea: {e}")
            continue
